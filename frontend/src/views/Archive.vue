    ...
    <div class="game-save-container">
      <div class="save-panel">
        ...
        <div class="panel-header">
          <button class="close-btn" title="Back to Login" @click="backHome">✖️</button>
          <h2 class="panel-title"><span class="title-icon">🎮</span> 当前游戏存档</h2>
        ...
        <div class="save-slots">
          ...
          >
            ...
            <div class="save-content">
              <div class="save-info">
                <h3 class="save-name">Save #{{ save.saveId }}</h3>
                <div class="save-details">
                  <span class="level">Score {{ save.playerScore }}</span>
                  <span class="location">Stamina {{ save.playerStamina }}</span>
                  <span class="location">Room {{ save.playerRoomId }}</span>
                </div>
                <div class="save-meta">
                  <span class="date">{{ formatDate(save.saveTime) }}</span>
                </div>
              </div>
            </div>
...
async function fetchSaves() {
  try {
    ...
    console.log('>> fetchSaves: 设置 selectedSave.value =', selectedSave.value)
  } catch (err) {
...
async function loadSave(saveId, idx) {
  try {
    ...
    console.log('>> loadSave: 更新 selectedSave.value =', selectedSave.value)
...
async function deleteSave(saveId) {
  ...
  try {
    console.log(`>> deleteSave: 调用 gameApi.delete({ save_id: ${saveId} }) …`)
    ...
    selectedSave.value = gameSaves.value.length > 0 ? 0 : null
    console.log('>> deleteSave: 更新 selectedSave.value =', selectedSave.value)
...
.panel-header {
  ...
  text-align: center;
  background: linear-gradient(135deg, rgba(180, 160, 140, 0.12), rgba(141, 110, 99, 0.12));
