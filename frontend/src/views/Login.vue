<template>
  <div class="login-page">
    ...
    <transition name="screen-fade" mode="out-in">
      <div v-if="showWelcome" key="welcome" class="welcome-screen">
        <div class="welcome-content">
          <img :src="`${baseUrl}images/logo.png`" class="welcome-logo" alt="Game Logo" />
    ...
    <transition name="screen-fade" mode="out-in">
      ...
      >
        <!-- Close button -->
        <button class="close-button" :disabled="isLoading" title="返回主页" @click="backToWelcome">
          ×
        </button>

        <!-- Left visual panel -->
        <div v-show="!isMobile" class="login-image">
          <img :src="`${baseUrl}images/login-bg.png`" class="login-bg" alt="login splash" />
        </div>
        ...
        <transition name="fade-slide" mode="out-in">
          ...
          <div v-if="!isRegisterMode" key="login" class="login-form">
            ...
            <!-- ---- Jump‑to‑register ---- -->
            <button class="register-button" :disabled="isLoading" @click="toggleMode">
              注册新账号
            </button>
          </div>
...
async function handleRegister() {
  if (!registerData.username || !registerData.password) return alert('请完整填写表单')
  if (registerData.password !== registerData.confirm) return alert('两次输入的密码不一致')
  isLoading.value = true
  try {
    ...
    console.log('Sending registration data:', {
      ...
      avatarFile: avatarFile.value?.name ?? 'none',
      fileSize: avatarFile.value ? (avatarFile.value.size / 1024).toFixed(2) + 'KB' : 'none',
    })
...
.welcome-btn.blue-effect {
  ...
  box-shadow:
    0 0 20px rgba(180, 160, 140, 0.5),
    0 10px 25px rgba(0, 0, 0, 0.3);
  transform: translateY(-3px) scale(1.05);
}

.start-btn:hover {
  background: linear-gradient(135deg, rgba(180, 160, 140, 0.3), rgba(141, 110, 99, 0.3));
}
