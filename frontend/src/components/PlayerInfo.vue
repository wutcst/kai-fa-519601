<template>
  <div class="container">
    <div class="hud-top">
      ...
      <div class="player-panel glass-card">
        <div class="panel-top">
          ...
          <div class="player-actions">
            <button class="exit-btn" title="Exit Game" @click="exitGame">退出</button>
            <button class="backpack-btn" title="Backpack (B)" @click="toggleBackpack">
              <span class="backpack-btn-icon">🎒</span>
              <span class="backpack-counter">
                {{ getCurrentWeight() }}/{{ backpackInfo.backpackSize }}
              </span>
            </button>
          </div>
        </div>
      ...
      <div :class="['leaderboard-card glass-card', { collapsed: !showLeaderboard }]">
        <h3 class="leaderboard-title">在线用户</h3>

        <!-- 在线用户列表 -->
        <transition name="lb">
          <ul v-show="showLeaderboard" class="leaderboard-list">
            <li v-for="(p, idx) in sortedPlayers" :key="p.id" class="leaderboard-item">
        ...
        <!-- 底部折叠按钮 -->
        <button class="toggle-btn" aria-label="切换在线用户列表" @click="toggleLeaderboard">
    ...
    <!-- 保存存档按钮（固定在右下角） -->
    <button class="save-btn" title="保存存档" @click="saveGame">保存存档</button>
    ...
    <teleport to="body">
      <div v-if="showBackpack" class="backpack-modal">
        <div class="backpack-header">
          <h3>背包</h3>
          <button class="close-btn" @click="showBackpack = false">✕</button>
        </div>
        <div class="backpack-content">
          <div class="backpack-info">
            <span class="backpack-capacity"
              >当前重量：{{ getCurrentWeight() }}/{{ backpackInfo.backpackSize }}</span
            >
            <span class="backpack-value">总价值：{{ getTotalValue() }}</span>
          </div>
          <div v-if="backpackInfo.itemList.length === 0" class="empty-backpack">
            背包是空的，快去收集物品吧！
          </div>
            ...
            >
              ...
              <div class="backpack-item-details">
                ...
                <div class="backpack-item-meta">
                  <span class="item-size">重量: {{ item.itemSize }}</span
                  ><span class="item-value">价值: {{ item.itemValue }}</span>
                </div>
              </div>
              <!-- 新增：如果是“魔法饼干”或“体力药水”，则显示“使用”按钮 -->
              <button
                v-if="item.itemName === '魔法饼干' || item.itemName === '体力药水'"
                class="use-item-btn"
                @click="useItem(item)"
              >
                使用
              </button>
              <button class="drop-item-btn" @click="dropItem(item)" title="丢弃">
                <svg
                  width="16"
                  height="16"
                  viewBox="0 0 24 24"
...
const getCurrentWeight = () => backpackInfo.itemList.reduce((s, i) => s + i.itemSize, 0)

const getTotalValue = () => backpackInfo.itemList.reduce((s, i) => s + i.itemValue, 0)
...
async function fetchPlayers() {
  try {
    ...
    if (code === 200) {
      ...
      console.warn('[fetchPlayers] 后端返回异常，code:', code, 'message:', message)
    }
  } catch (err) {
    // 5. 捕获网络或其它错误
...
async function dropItem(item: {
  ...
  try {
    ...
    console.log('[dropItem] 开始调用 backpackApi.dropItem，itemId =', item.itemId)
    ...
    console.log('[dropItem] 触发事件 bus.emit("item-dropped")，通知房间更新')
  ...
  try {
    ...
    if (code === 0 || code === 200) {
      ...
      if (item.itemName === '魔法饼干') {
        ...
        const recover = 20
        const newHealth = Math.min(player.maxHealth, player.health + recover)
...
.glass-card::before {
  ...
  padding: 2px; /* 描边粗细 */
  background: linear-gradient(135deg, rgba(180, 160, 140, 0.18), rgba(180, 160, 140, 0.04));
  -webkit-mask:
...
.toggle-btn {
  ...
  background: linear-gradient(135deg, rgba(180, 160, 140, 0.12), rgba(180, 160, 140, 0.06));
  border: 1px solid rgba(180, 160, 140, 0.2);
  border-radius: 24px;
  padding: 8px 16px;
  min-width: 80px;
...
.toggle-btn:hover {
  background: linear-gradient(135deg, rgba(180, 160, 140, 0.18), rgba(180, 160, 140, 0.1));
